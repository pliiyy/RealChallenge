...
class DekanController extends Controller
{
    ...
    {
        $query = Dekan::query();
        $query->with(["fakultas",'user.biodata']);

        // Searching berdasarkan nama
        if ($request->filled('search')) {
            $query->where('fakultas.nama', 'like', '%'.$request->search.'%')
            ->orWhere('user.biodata.nama', 'like', '%'.$request->search.'%')
        ...
        $users = User::with(['biodata'])->doesntHave("dekan")->get();

        return view('dekan', compact('dekan','users'));
    }
    ...
    {
        // 1. Validasi Gabungan: Data User (optional/required) dan Data Dekan
        ...
        try {
            ...
            $message = 'User Dekan Fakultas berhasil dtambahkan!';
            
            return redirect('/dekan')->with('success', $message);

        } catch (\Exception $e) {
            DB::rollBack();
            // Log error untuk debug
            
            return redirect()->back()->withInput()->with('error', 'Terjadi kesalahan saat menyimpan data Dekan. ' . $e->getMessage());
        }
    ...
    public function destroy($id)
    {
        ...
        return redirect('/dekan')->with('success', 'Dekan berhasil dihapus!');
    }
}
