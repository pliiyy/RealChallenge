    ...
    <div class="col-lg-10 col-md-9 p-4">
        <div class="profile-card mx-auto" style="max-width: 800px;">
            ...
            <div class="profile-header text-center py-4 bg-primary text-white rounded-top">
                ...
                <form id="formUploadFoto" method="POST" action="{{ route('profil.updateFoto') }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="file" name="foto" id="fotoInput" accept="image/*" style="display: none;"
                        onchange="previewFoto(event)">
                    
                    <div class="d-inline-block mb-3 position-relative" style="cursor: pointer;" onclick="document.getElementById('fotoInput').click()">
                        <div id="previewContainer" class="rounded-circle bg-light d-flex align-items-center justify-content-center mx-auto"
                            style="width: 120px; height: 120px; border: 3px solid #fff; overflow: hidden;">
                            @if (auth()->user()->biodata->foto_profil)
                                <img id="fotoPreview" src="{{ asset('storage/foto_profil/' . auth()->user()->biodata->foto_profil) }}"
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-12 mt-3">
                            ...
                            <textarea class="form-control" rows="3" placeholder="Tuliskan sesuatu tentang dirimu..."
                                name="keterangan">{{ auth()->user()->Biodata->keterangan ?? '' }}</textarea>
                        </div>
                    </div>
        ...
        function previewFoto(event) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    previewContainer.innerHTML = `<img id="fotoPreview" src="${e.target.result}" class="w-100 h-100" style="object-fit: cover;">`;
